...
public class IssuePOJOAdaptor {
	...
	public IssuePOJO adapt( JiraIssue jIssue) {
		...
		issue.setAggregatetimeestimate(jIssue.getFields().getAggregatetimeestimate());
		issue.setAggregatetimeoriginalestimate(jIssue.getFields().getAggregatetimeoriginalestimate());
		...
		for ( int i = 0 ; i < jIssue.getFields().getSubtasks().length  ; i++ ) {
			subtasks[i] = new String( jIssue.getFields().getSubtasks()[i].getKey() ) ;
		}
