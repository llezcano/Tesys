...
public class JiraRESTClient {
    ...
    public static void main (String [] args) {
        JiraRESTClient jrc = new JiraRESTClient("http://ing.exa.unicen.edu.ar:8086/atlassian-jira-6.0/", "grodriguez", "654321" ) ;
        jrc.getIssueTypes();
    }
    ...
    //    URI jiraServerUri = URI.create( "http://ing.exa.unicen.edu.ar:8086/atlassian-jira-6.0/" );
        final AsynchronousJiraRestClientFactory factory = new AsynchronousJiraRestClientFactory();
        restClient = factory.createWithBasicHttpAuthentication(jiraServerUri, "grodriguez", "654321" );
        } catch( Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public List<IssueTypePOJO> getIssueTypes() {
        try {
            Iterable<IssueType> response = restClient.getMetadataClient().getIssueTypes().claim();
            List<IssueTypePOJO> issueTypes = new ArrayList<IssueTypePOJO>();
            for (IssueType type : response) {          
                issueTypes.add( new IssueTypePOJO(type.getId(), type.getName(), type.getDescription()) );
            }
            return issueTypes;
        } catch (Exception e){
