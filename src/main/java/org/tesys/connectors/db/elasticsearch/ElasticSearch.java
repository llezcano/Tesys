...
public class ElasticSearch {
    ...
    private Integer getSize( String index, String dtype ) {
        ...
        try {
            JsonNode jsonResponse = client.GET( pathToStore( index, dtype, QUERY ), param )
    ...
    private ArrayNode executeQuery( String index, String type ) {
        try {
            ArrayNode response = (ArrayNode) client.GET( this.pathToStore( index, type, QUERY ) )
                                                    .readEntity( JsonNode.class )
                                                    .get( "hits" ).get( "hits" );
            return response;
        } catch (Exception e) {
    ...
    private ArrayNode executeQuery( String index, String type, String query ) {
        try {
            ArrayNode response = (ArrayNode) client.POST( this.pathToStore( index, type, QUERY ), query )
                                                    .readEntity( JsonNode.class )
                                                    .get( "hits" ).get( "hits" );
            return response;
        } catch (Exception e) {
    ...
    public boolean isValidDeveloper( String name, String repoId ) {
        ...
        try {
            String query = "{ \"query\": { \"bool\": { \"must\": [ { \"match\": { \"scmUser\": \"" + name + "\" }}, { \"match\": {\"repository\": \"" + repoId + "\" }} ] } } }";
            JsonNode jsonResponse = client.POST( this.pathToStore( INDEX_SCM, DTYPE_USER, QUERY ), query )
                                            .readEntity( JsonNode.class );
