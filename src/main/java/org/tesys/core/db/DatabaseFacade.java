...
public class DatabaseFacade {
  ...
  /**
   * El constructor debe estar para cuando se llama la aplicacion desde el core y no desde rest
   */
   ...
   * @PUT
   * 
   * @Path("/config") public String changeConnectorLocation(String url) { try { client.setURL(url);
   * } catch (MalformedURLException e) { e.printStackTrace(); } return
   * Messages.getString("DatabaseFacade.urlchanged"); //$NON-NLS-1$ }
   */
