...
public class ProcessData {
  ...
  /**
   * Recolecta los diferentes tipos de issues, estos solo se sacan del project tracking
  ...
  /**
   * Recolecta todos los developers que existen (en el project tracking)
   * y los guarda en la base de datos indicando que issues corresponde a cada
   * uno, pera esto es necesario que primero se computen los issues sino 
   * se van a guardar issues de un analisis anterior
   */
  ...
  /**
   * Se obtienen y almacenan todas las metricas que puede manejar tesys
   * se utiliza un agregador para poder recorrer todos los programas que esten
   * conectados
   */
  ...
  /**
   * Se obtienen los issues desde el project tracking con la informacion asociada
   * y se utiliza un agregador para poder obtener todos los valores de metricas
   * que tenga cada programa para ofrecer
   */

  private void processIssues(ProjectTracking pt, Aggregator aggregator) {
    //Aca se traen todos los issues y se agregan a mano los datos desde el project tracking
    List<IssueInterface> issues = Arrays.asList( pt.getIssues() );
    
    /**
     * La manera correcta de hacer seria trayendo solo las key, y que el agregator meta
     * las metricas que corresponden, pero por problemas de performance se realiza de esta manera
     */
    ...
    for( IssueInterface i : issues ) {
      ...
      issueActual.addMetric("progress", Double.valueOf( i.getProgress().getProgress()));
      issueActual.addMetric("estimated", Double.valueOf( i.getProgress().getTotal() ));
