...
public class SvnRevisions {
   ...
   */
  public SvnRevisions(Client client) {
    try {
      datosEnServer =
          client.prepareSearch(DBUtilities.ES_SVN_INDEX)
              .setTypes(DBUtilities.ES_REVISIONS_DATATYPE).execute().actionGet();
    } catch (ElasticsearchException e) {
      System.err.println(Messages.getString("servernotfound") + e.getMessage()); //$NON-NLS-1$
      System.exit(1);
    }
  ...
  public List<JsonNode> getDataInJsonFormat(SearchResponse data) {
    ...
    JsonParser jp;
    JsonNode jsonRevision = null;
    try {
      jp = factory.createJsonParser(datosEnServer.toString());
      jsonRevision = mapper.readTree(jp);
    } catch (Exception e) {
    }
    ...
    for (int i = 0; i < jsonRevision.get(DBUtilities.ES_HITS_TAG)
        .get(DBUtilities.ES_HITS_TAG).size(); i++) {
      revisions
          .add(jsonRevision.get(DBUtilities.ES_HITS_TAG)
              .get(DBUtilities.ES_HITS_TAG).get(i)
  ...
  public List<Revision> getRevisions(List<JsonNode> revisions) {
    ...
    for (JsonNode jsonNode : revisions) {
      ...
      SimpleDateFormat dateFormat =
          new SimpleDateFormat(DBUtilities.SVN_DATE_FORMAT_IN_ES);

      mapper.setDateFormat(dateFormat);

      Revision r = null;
      try {
        r = mapper.readValue(jsonNode, Revision.class);
      } catch (Exception e) {
      }
    ...
    Collections.sort(result); // se ordenan las revisiones por fecha
    return result;
