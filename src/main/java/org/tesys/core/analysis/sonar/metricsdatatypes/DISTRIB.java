 ...
 * eso indica que hay un archivo de complejidad 0 y de las demas complejidades no hay archivos
 * (aunque depende la metrica para conocer su significado, esa es la interpertacion)
 */
...
public class DISTRIB implements Metrics {
   ...
   * negativos indicando que saco a archivo o lo que fuera de esa categoria y probablemente este en
  ...
  public String getNewAnalysisPerTask() {
    ...
    if (actCom.length == antCom.length) {
      ...
      for (int i = 0; i < actCom.length; i++) {
        String[] aux = actCom[i].split(EQUAL);
        ret.append(aux[0] + EQUAL
            + (Integer.parseInt(aux[1]) + Integer.parseInt(antCom[i].split(EQUAL)[1])) + SEMI_COLON);
      }
      return ret.substring(0, ret.length() - 1);
    }
