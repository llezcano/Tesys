...
public class SonarAnalizer {
	...
	for (AnalisisPOJO analisisAcumuladoActual : analisisAcumulados) {
	    ...
	    // esa ruta
	    String path = analisisAcumuladoActual.getRevision().getPath();

	    // los id de los ultimos analisis son el path en md5
	    if (ultimoAnalisisDAO.exists(MD5.generateId(path))) {
		LOG.log(Level.INFO, "Se recupera analisis anterior para " + path);

		AnalisisPOJO analisisAcumuladoPrevio = ultimoAnalisisDAO
			.read(MD5.generateId(path));
		
		analisisPorCommit.add( this.calcularDiferenciasDeAnalisis(analisisAcumuladoPrevio, analisisAcumuladoActual, metricas)  );
	    //si no existe un analisis previo de la ruta..
	    } else {

		RevisionByOriginalIdentifierQuery revQuery = new RevisionByOriginalIdentifierQuery(
			analisisAcumuladoActual.getRevision().getRevision(),
			analisisAcumuladoActual.getRevision().getRepository());
		
		// se busca si existe una revision previa a la actual
		RevisionPOJO lastRevision = revQuery.execute();
		if (lastRevision != null) {
		    
		    LOG.log(Level.INFO, "Como no existe un analisis previo del path " + path + 
			    " se va a analizar todo el repositorio");
		    
		    // TODO se deberian correr analisis de todo el repositorio de la rev esta y anterior
		    //para poder comparar y guardadr el analisisAcumuladoActual como ultimo
		    
		    
		} else {
		    LOG.log(Level.INFO, "La revision en " + path + "no pareciera tener un antencesor"
		    	+ "por lo que sera tomada como base para los siguintes analisis");
		}
		//si no hay revisiones previas no se puede guardar como analisis por commit porque no
		// hay certaza de que haya codigo viejo (caso de commits que no estan dentro del
		// sistema pero si en el scm)

	    }

	    ultimoAnalisisDAO.update(MD5.generateId(path), analisisAcumuladoActual);
